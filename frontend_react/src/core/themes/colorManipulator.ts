 ...
 * @beta
 */
function hexToRgb(colorParam: string) {
  const color = colorParam.slice(1);
 ...
 * @beta
 */
function decomposeColor(color: string | DecomposeColor): DecomposeColor {
  // Idempotent
...
export function getLuminance(color: string, background?: string) {
  ...
  if (background && parts.type === 'rgba') {
    ...
    const alphaValue = rgb[3];
    rgb[0] = rgb[0] * alphaValue + backgroundParts.values[0] * (1 - alphaValue);
    rgb[1] = rgb[1] * alphaValue + backgroundParts.values[1] * (1 - alphaValue);
    rgb[2] = rgb[2] * alphaValue + backgroundParts.values[2] * (1 - alphaValue);
  }

  const rgbNumbers = rgb.map((valParam: any) => {
    let val = valParam;
    if (parts.type !== 'color') {
      val /= 255; // normalized
    }
  ...
  return Number(
    (0.2126 * rgbNumbers[0] + 0.7152 * rgbNumbers[1] + 0.0722 * rgbNumbers[2]).toFixed(3),
  );
}
 ...
 * @beta
 */
export function darken(color: string, coefficientParam: number) {
  const parts = decomposeColor(color);
  const coefficient = clamp(coefficientParam);
 ...
 * @beta
 */
export function lighten(color: string, coefficientParam: number) {
  const parts = decomposeColor(color);
  const coefficient = clamp(coefficientParam);
 ...
 * @beta
 */
export function alpha(colorParam: string, valueParam: number) {
  let color = colorParam;
  if (color === '') {
    return '#000000';
  }
  const value = clamp(valueParam);
